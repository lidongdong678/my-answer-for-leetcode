...
function expressiveWords(S, words) {
    let res = 0;
    const sArr = splitWords(S);
    const len = sArr.length;


    words.forEach(word => {
        const wordArr = splitWords(word);
        if (wordArr.length !== len) return null;
        ...
        for (let i = 0; i < len; i++) {
            ...
            if (sItem.length > 2) {
                if (sItem.length < wItem.length || sItem[0] !== sItem[0]) return null
            } else {
                if (sItem !== wItem) return null
            }
        }
...
function splitWords(S) {
    ...
    for (let i = 1; i < len + 1; i++) {
        const currectChar = S[i];
        if (currectChar === lastChar) {
            addStr += currectChar;
        } else {
            res.push(addStr);
            addStr = currectChar;
            lastChar = currectChar;
        }
...
function test(S, words) {
    ...
    const result = expressiveWords(S, words);
    const end = new Date().getTime();
    const during = end - start;
    console.log(`输入：${S}和${words}， 输出：${result}， 耗时：${during}`)
}


// 运行测试
test("heeellooo", ["heeeello", "hi", "helo"]);
test("hiiii", ["hhhhhhi", "hhi", "hhiiiiiii"]);
test("aaa", ["aaaa"]);
